  ...
  switch (action.type) {
    ...
    case SET_DISPLAY_MONTH:
      return {
        ...
        displayedEntries: state.entries.filter(
          entry => monthByYear(entry.created) === action.month
        )
      };
    ...
    case DELETE_ENTRY:
      return {
        ...state,
        displayedEntries: state.displayedEntries.filter(
          entry => entry.entry_id !== parseInt(action.entryId)
        ),
        isDeleting: action.isDeleting
      };
    default:
